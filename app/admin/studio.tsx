...
export function AdminStudio({ paymentsReady }: { paymentsReady: boolean }) {
 ...
 {tab === 'Messages' && <section className="studio-inbox"><h2>Customer messages</h2><p>The About form prepares an email addressed to natspottery@gmail.com. Earlier saved studio messages appear below.</p>{messages.length === 0 && <p>No saved studio messages.</p>}{messages.map(m => <article key={m.id}><div className="collection-heading"><h3>{m.topic}</h3><span>{m.is_read ? 'Read' : 'New'}</span></div><p><strong>{m.name}</strong> · <a href={`mailto:${m.email}`}>{m.email}</a></p><p className="preserve-lines">{m.message}</p><small>{new Date(m.created_at).toLocaleString()}</small><div className="button-row"><a className="paper-button" href={`mailto:${m.email}?subject=${encodeURIComponent('Re: '+m.topic+' — Pottery by Natalie')}`}>Reply by email</a>{!m.is_read && <button className="text-button" onClick={() => markRead(m.id)}>Mark read</button>}</div></article>)}</section>}
