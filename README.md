...
**Studio login and persistent product editing are configured on Netlify. Review submissions still require the private Supabase server key. Contact email delivery uses FormSubmit's free background form endpoint. Payments remain disabled; Venmo through PayPal Checkout is the owner's requested next integration.**

Until Supabase is configured, the shop reads the supplied collection from `app/initial-products.json`, the studio explains the setup requirement, and checkout is disabled. Contact form delivery uses a separate free form endpoint. With Supabase configured, the database becomes authoritative; database failures show unavailable states rather than stale purchaseable fallback data.
...
6. Open `/admin`, sign in, confirm all prices and stock counts, and add Natalie’s public contact email if desired. The About page uses natspottery@gmail.com for contact. New contact form messages are delivered through FormSubmit to that address; the studio Messages tab retains earlier database messages. Natalie can reply directly because the customer's email becomes Reply-To.
...
The full application is deployed at **https://pottery-by-natalie.netlify.app/**. The Netlify project is linked to this repository and automatically builds `main`. Production is public; non-production previews require Netlify team login. Studio authentication is configured and the first approved owner has been invited to choose a password. Reviews and payments require the private server settings described below. Contact delivery runs through FormSubmit in the background.
...
1. The About page submits by AJAX to FormSubmit, so visitors remain on the website and receive an inline success or error message.
2. Submit the form once after deployment. FormSubmit emails **natspottery@gmail.com** an activation link; the mailbox owner must click that link before customer messages are delivered.
3. The form sends name, email, topic, and message with a honeypot, table formatting, and the subject **Pottery by Natalie — New contact message**. The email field becomes Reply-To.
4. FormSubmit is free and requires no account or payment information. It retains submissions for 30 days. The old database-only `/api/contact` route remains removed, and review submission setup is unchanged.
...
## Venmo connection still needed

The requested customer payment method is Venmo, using a US PayPal Business account and the Venmo option in PayPal Checkout for verified website payments. Do not enable the current Stripe checkout as a substitute or label a PayPal payment link as automatic fulfillment. Connect the seller's PayPal Business account through a server integration with PayPal Orders: create and capture orders using database prices, verify payment identity/amount/currency, reconcile verified webhooks, and decrement stock exactly once. Only a confirmed payment removes the last piece from the collection. Start with sandbox credentials and verify last-item reservations, cancellation, duplicate notifications, and successful capture before live payments.

Use PayPal's merchant payment notifications for sale emails once Natalie confirms the receiving account and notification email. Her merchant account and shipping or pickup choices have not been supplied. The contact destination natspottery@gmail.com is not automatically a merchant payment notification address. No PayPal payments or sale emails have been tested or activated.
